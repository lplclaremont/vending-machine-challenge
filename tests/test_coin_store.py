...
"""
def test_initially():
    machine = CoinStore([7,6,5,4,3,2,1,0])
...
def test_invalid_coin_quantity():
    with pytest.raises(TypeError) as err_info:
        machine = CoinStore([-2, 1, 1, 1, 1, 1, 1, 1])
...
"""
def test_multiple_coins_and_diff_denominations_given():
    machine = CoinStore([20,20,20,1,20,20,20,20])
    machine.deposit(100)
...
def test_complicated_example():
    machine = CoinStore([20,20,10,1,3,0,1,4])
    machine.deposit(200)
    machine.deposit(100)
...
def test_error_when_out_of_coins():
    machine = CoinStore([0,0,0,0,0,0,0,1])
    machine.deposit(5)
    with pytest.raises(ValueError) as err_info:
